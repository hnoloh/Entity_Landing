...
describe('Admin Authorization', () => {
  ...
  it('Unit - autorización inválida', async () => {
    const res = await simulateRequest('/api/registrations', 'GET', { authorization: 'Bearer invalid-token' });
    expect(res.statusCode).toBe(401);
    expect(JSON.parse(res.body).error).toBe('No autorizado');
  });
  ...
  it('Unit - autorización válida', async () => {
    ...
    const res = await simulateRequest('/api/registrations', 'GET', { authorization: `Bearer ${token}` });
    expect(res.statusCode).toBe(200);
  });

  it('Integration - modificación autorizada', async () => {
    const token = process.env.VITE_ADMIN_SECRET || 'admin-secret-2026';
    const res = await simulateRequest('/api/registrations/status', 'POST', { authorization: `Bearer ${token}` }, { email: 'nonexistent@test.com', status: 'Approved' });
    expect(res.statusCode).toBe(404);
  });

  it('Integration - modificación denegada', async () => {
    const res = await simulateRequest('/api/registrations/status', 'POST', { authorization: 'Bearer fake' }, { email: 'test', status: 'Approved' });
    expect(res.statusCode).toBe(401);
  });
  
  it('Integration - registro publico inalterado sin auth', async () => {
    const res = await simulateRequest('/api/register', 'POST', {}, { email: 'new-auth-test-' + Date.now() + '-' + Math.random() + '@test.com' });
