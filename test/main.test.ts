...
describe("Email Confirmation Dispatch (FIA-056)", () => {
  ...
  it("should dispatch confirmation email and record evidence upon valid registration", async () => {
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          ...
          resolve({
            ...
            json: async () => ({
              message:
                "¡Solicitud enviada con éxito! Te hemos añadido a la lista de espera.",
            }),
          });
        }, 100);
      });
    ...
    fetch("/api/register", {
      ...
      body: JSON.stringify({ email: "test.confirm@entity.test" }),
    });
  ...
  it("should not dispatch email when registration fails with HTTP 409 (duplicate)", async () => {
    ...
    await fetch("/api/register", {
      method: "POST",
      body: JSON.stringify({ email: "duplicate@entity.test" }),
    });
  ...
  it("should not dispatch email if server persistence fails", async () => {
    ...
    await fetch("/api/register", {
      method: "POST",
      body: JSON.stringify({ email: "error@entity.test" }),
    });
    expect(fs.existsSync(sentEmailsPath)).toBe(false);
  });
  ...
  it("should ensure sent email content is coherent with the preview", async () => {
    ...
    expect(document.getElementById("preview-subject")?.textContent).toBe(
      mockEmailLog.subject,
    );
    expect(document.getElementById("preview-preheader")?.textContent).toBe(
      mockEmailLog.preheader,
    );
    expect(
      document
        .getElementById("preview-body")
        ?.textContent?.replace(/\s+/g, " "),
    ).toContain(
      "Nos alegra informarte que tu solicitud para acceder a la beta privada de Entity ha sido aceptada.",
    );
    expect(document.getElementById("preview-cta")?.textContent).toBe(
      mockEmailLog.cta,
    );
    expect(
      document
        .getElementById("preview-footer")
        ?.textContent?.replace(/\s+/g, " "),
    ).toContain("waitlist privada de Entity. © 2026 Entity");
  });
});
...
describe("Email Invitation Preview (FIA-058)", () => {
  ...
  it("should render the email invitation preview correctly in admin view (FIA-058)", async () => {
    ...
    expect(subject).not.toBeNull();
    expect(subject?.textContent).toContain(
      "¡Has sido invitado a la Beta Privada de Entity!",
    );

    const preheader = document.getElementById("invitation-preheader");
    expect(preheader).not.toBeNull();
    expect(preheader?.textContent).toContain(
      "Tu invitación exclusiva para unirte al Workspace inteligente de Entity ya está aquí.",
    );

    const body = document.getElementById("invitation-body");
    expect(body).not.toBeNull();
    expect(body?.textContent).toContain(
      "invitarte a probar de forma prioritaria la beta privada",
    );
    ...
    expect(footer).not.toBeNull();
    expect(footer?.textContent).toContain(
      "invitación exclusiva para probar la beta privada de Entity. © 2026 Entity",
    );
  });
...
describe("Email Invitation Dispatch (FIA-059)", () => {
  ...
  it("should display error message on status container when invitation endpoint returns an error (FIA-059)", async () => {
    ...
    const errorContainer = document.getElementById("admin-status-container");
    expect(errorContainer?.textContent).toContain(
      "Fallo al despachar la invitación.",
    );
  });
...
describe("Metrics Dashboard (FIA-061)", () => {
  ...
  it("should render metrics accurately based on payload data", async () => {
    ...
    // Invitaciones
    expect(
      document.getElementById("metric-invite-sent")?.textContent,
    ).toContain("1");
    expect(
      document.getElementById("metric-invite-pending")?.textContent,
    ).toContain("1");
    expect(
      document.getElementById("metric-invite-error")?.textContent,
    ).toContain("1");
