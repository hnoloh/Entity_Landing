...
describe('Build + Deploy Smoke Test', () => {
  beforeAll(async () => {
    ...
    execSync('npm run build', {
      ...
      env: { ...process.env, VITE_API_BASE_URL: `http://127.0.0.1:${BACKEND_PORT}` },
      stdio: 'inherit'
    });
    ...
    backendProcess = spawn('npx', ['tsx', 'src/server.ts'], {
      cwd: path.join(__dirname, '..'),
      env: { ...process.env, PORT: BACKEND_PORT.toString(), VITE_ADMIN_SECRET: 'admin-secret-2026' },
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      backendProcess.on('close', (code) => {
        if (!ready) reject(new Error(`Backend exit with code ${code} before ready. Stderr: ${stderrOutput}`));
      });
      setTimeout(() => {
  ...
  it('Smoke Test: Frontend desplegado y Backend runtime conectado', async () => {
    ...
    const frontRes = await fetch(`http://127.0.0.1:${FRONTEND_PORT}/index.html`);
    ...
    const adminRes = await originalFetch(`http://127.0.0.1:${BACKEND_PORT}/api/registrations`, {
    ...
    const adminResNoToken = await originalFetch(`http://127.0.0.1:${BACKEND_PORT}/api/registrations`);
    expect(adminResNoToken.status).toBe(401);

    // 6. Smoke Test Baja
    const unsubRes = await originalFetch(`http://127.0.0.1:${BACKEND_PORT}/api/registrations/unsubscribe`, {
      method: 'POST',
    ...
    const adminResAfter = await originalFetch(`http://127.0.0.1:${BACKEND_PORT}/api/registrations`, {
      headers: { 'Authorization': 'Bearer admin-secret-2026' }
    });
