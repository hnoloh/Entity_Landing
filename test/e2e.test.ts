...
describe("QA E2E Productivo", () => {
    ...
    globalThis.fetch = async (input, init) => {
      ...
      if (urlStr.startsWith("http://127.0.0.1:3000")) {
        return originalFetch.call(
          globalThis,
          urlStr.replace("http://127.0.0.1:3000", baseUrl),
          init,
        );
      }
  ...
  it("Debe ejecutar el flujo completo E2E integrado sin mocks", async () => {
    ...
    if (!mainJs)
      throw new Error(
        "No se encontró el bundle principal en dist/assets. ¿Se ejecutó npm run build?",
      );
    ...
    const adminJs = files.find(
      (f) => f.startsWith("admin-") && f.endsWith(".js"),
    );
    if (!adminJs)
      throw new Error("No se encontró el bundle admin en dist/assets.");
