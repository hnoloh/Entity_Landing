...
async function dispatchEmail(emailData: {
  to: string;
  subject: string;
  preheader: string;
  body: string;
      ...
      try {
        ...
        if (email === "qa.mailerror@entity.test") {
          ...
          try {
            ...
            emailStatus = "error";
            emailError =
              err instanceof Error ? err.message : "SMTP connection failure.";
          }
        }
      ...
      try {
        ...
        if (registration.unsubscribed) {
          ...
          res.end(
            JSON.stringify({
              error:
                "El usuario se ha dado de baja y no puede recibir más comunicaciones.",
            }),
          );
          return;
        }
